...
var mcpServeCmd = &cobra.Command{
	Use:   "serve",
	Short: "Run k8x as an MCP server",
	Long: `Run k8x as a Model Context Protocol server, exposing its shell execution
tools to other MCP clients. This allows other applications to use k8x's
Kubernetes command execution capabilities.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Create MCP server with shell execution tools
		workDir, _ := os.Getwd()
		server := mcp.NewMCPServer(workDir, &cfg.Kubernetes)
		...
		fmt.Fprintf(os.Stderr, "Starting k8x MCP server...\n")
		fmt.Fprintf(os.Stderr, "Exposing shell execution tools via MCP protocol\n")
