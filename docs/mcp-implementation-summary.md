...
- Completely rewritten to use mcp-go's `server.MCPServer`
- Exposes k8x's shell execution capabilities via mcp-go's tool registration API
- Uses mcp-go's `NewTool` and server tool handlers
- Secure shell execution with safety restrictions maintained
- Simplified implementation with `ServeStdio` function
...
#### MCP Server Commands (`cmd/mcp.go`)

```bash
k8x mcp serve                          # Run k8x as MCP server
```
...
### Scenario 2: k8x as MCP Server

Other applications can use k8x's Kubernetes expertise:

```bash
# Terminal 1: Start k8x MCP server
k8x mcp serve

# Terminal 2: Configure another MCP client to use k8x
# (Example: Claude Desktop, Continue.dev, etc.)
```

### Scenario 3: Multi-Cluster Operations

Use multiple k8x instances for different clusters:
